...
class Handler:
    def __init__(self, n_start: int = 64, bias: str = 'random', seed: int = 42,
                 dataset: str = 'ALDH1', init_pos_count: int = None) -> None:

        assert bias in ['random', 'real', 'small', 'large'], "'bias' has to be either 'random', 'small', or 'large'"
        ...
        self.train_idx, self.screen_idx = self.get_start_data(
            n_start=n_start,
            bias=bias,
            seed=seed,
    ...
    def get_start_data(self, n_start: int = 64, bias: str = 'random', seed: int = 0,
                       init_pos_count: int = None) -> (np.ndarray, np.ndarray):
        ...
        if bias == 'random':
            ...
            hit_num = 10 if init_pos_count is None else int(init_pos_count)
            if hit_num < 0:
                raise ValueError('init_pos_count has to be >= 0')
            if hit_num > n_start:
                raise ValueError('init_pos_count has to be <= n_start')

            hits_idx = np.where(self.all_y == 1)[0]
            if hit_num > len(hits_idx):
                raise ValueError('init_pos_count exceeds available positive samples')

            nonhit_num = n_start - hit_num
            nonhits_idx = np.where(self.all_y == 0)[0]
            if nonhit_num > len(nonhits_idx):
                raise ValueError('Not enough negative samples for requested init_pos_count')

            if hit_num == 0:
                selected_hit_idx = np.array([], dtype=int)
            else:
                selected_hit_idx = np.array(rng.choice(hits_idx, size=hit_num, replace=False))

            # get the other nonhit molecules
            if nonhit_num == 0:
                selected_others_idx = np.array([], dtype=int)
            else:
                selected_others_idx = np.array(rng.choice(nonhits_idx, size=nonhit_num, replace=False))
