...
# CSV Method column → display label
method_labels = {
    "Exploitation": "Exploitation",
    "dynamiccomb (95:5)": "Combo (95:5)",
    "dynamiccomb (90:10)": "Combo (90:10)",
    "dynamiccomb (80:20)": "Combo (80:20)",
}
strategy_order = list(method_labels.keys())
strategy_labels = list(method_labels.values())
...
data_dir = os.path.join(_RESULTS, "260323_dynamiccomb_ratio_experiments")
output_root = os.path.join(_FIGURES, "metric_bars_combo_output")
    ...
    with open(filepath) as f:
        ...
        for row in reader:
            if len(row) <= col_idx["Method"]:
                continue
            method = row[col_idx["Method"]].strip()
...
for dataset in datasets:
    csv_path = os.path.join(
        data_dir, f"{dataset}_metrics_results", f"{dataset}_cycle15_summary.csv")
    ...
    for method_name in strategy_order:
        if method_name in raw_data:
            for metric in metric_map:
                m, s = raw_data[method_name][metric]
    ...
    for metric in metric_order:
        ...
        if metric == "Cumulative Hits":
            y_min = (all_vals - all_errs).min() - 80
            y_max = (all_vals + all_errs).max() + 50
        else:
            y_min = (all_vals - all_errs).min() - 0.03
            y_max = (all_vals + all_errs).max() + 0.02
        ax.set_ylim(y_min, y_max)

        plt.tight_layout()
        plt.savefig(f"{out_dir}/Bar_{metric.replace(' ', '_')}.png", dpi=600, bbox_inches="tight")
        plt.savefig(f"{out_dir}/Bar_{metric.replace(' ', '_')}.pdf", bbox_inches="tight")
        plt.close()
